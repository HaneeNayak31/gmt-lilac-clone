...
const HeroSection = () => {
  ...
  return (
    <section ref={elementRef} className="w-full bg-[var(--color-background)] text-[var(--color-text-primary)] overflow-hidden">
      <div className="mx-auto lg:ml-40 pl-0 max-w-[1500px] ">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[50%_50%] items-center">
          ...
          <div className="flex flex-col items-center xl:items-start text-center justify-center xl:mr-49 mb-10 xl:mb-5 px-4 mt-8 xl:mt-0">
            <h1 className={`text-[40px] md:text-[50px] xl:text-[79px] leading-[1.1] xl:leading-[1] font-medium tracking-medium mb-8 whitespace-normal xl:whitespace-pre-wrap transition-all duration-[800ms] ease-out delay-[80ms] max-w-3xl xl:max-w-8xl ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
              Find clarity, <br className="hidden xl:block" />
              connection, & calm.
            </h1>

            <p className={`text-[16px] sm:text-[20px] self-center mb-10 ${isVisible ? 'slideIn delay-100' : 'preSlide'}`}>
              Clinical Psychologist | Santa Monica, CA
            </p>
            
            <div className={`flex justify-center self-center xl:mr-10 ${isVisible ? 'slideIn delay-200' : 'preSlide'}`}>
              <Link 
                href="/contact" 
